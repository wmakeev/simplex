...
- [x] Eval-free backend `interpret()` (entry `simplex-lang/interpret`) — tree-walking interpreter for environments without `new Function`/`eval` (strict CSP, MV3 extensions, Cloudflare Workers, Deno Deploy). Shares `runtime.ts` + `validate.ts` with `compile()`; kept in sync via parity tests.
