...
- **Eval-free backend for CSP / edge runtimes.** A second backend (`simplex-lang/interpret`) evaluates the AST with a tree-walker and needs no `new Function`/`eval` — it runs under a strict CSP, in MV3 extension service workers, Cloudflare Workers, and Deno Deploy. Identical language semantics (shared `runtime.ts` + `validate.ts`); a parity test suite is the synchronization gate. This removes the hard dependency on dynamic code generation that would otherwise bar SimplEx from exactly the sandboxed/edge deployments §2 targets — a fallback (slower per call), not a replacement for `compile()`.
